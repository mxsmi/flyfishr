...
inputControlsUI <- function(id) {
  ...
  tagList(
    fluidRow(
      column(1),
      ### First button to choose a state
      column(3,
    ...
    fluidRow(
      column(1),
      ### Second button to choose a USGS monitoring site
      column(3,
    ...
    ### Cite data source: (USGS data pulled using the dataRetrieval R package)
...
inputControlsServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    ### Reactive value which stores the sites with discharge data for the currently
    ### selected state
    sites <- reactiveVal()
    ### Reactive value which is TRUE if 'sites' is empty or FALSE if it is non-empty
    ...
    observeEvent(input$state, {
      ...
      waiter <- waiter::Waiter$new(color = "#00699480")$show()
      on.exit(waiter$hide())
      ...
      showNotification(paste("Data succesfully loaded for", input$state,
                             ". Select a USGS monitoring site from the drop-down menu."),
                       type = "message")
    })
